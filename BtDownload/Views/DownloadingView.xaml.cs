...
namespace BtDownload.Views
{
    ...
    public sealed partial class DownloadingView : Page
    {
        ...
        public async void FinishedDownload(DownloadInfo downloadInfo)
        {
            ...
            DownloadedVM.DownloadedInfoList.Add(downloadedInfo);

            //存储到数据库中
            DownloadedView.StoreDownloadedInfo(downloadedInfo);
        }
        ...
        private async void btnAdd_Click(object sender, RoutedEventArgs e)
        {
            ...
            var uri=GetUri(this.tbUri.Text);
            var filename = GetFileName(this.tbUri.Text);

            //创建文件
            //td:此处添加文件存在检测逻辑
            //
            var file = await defaultFolder.CreateFileAsync(filename, CreationCollisionOption.GenerateUniqueName);
